...
The package includes pre-parsed datasets, such as the DEA's Technology Catalogue for energy storage, together with the parsers that produce them (see the [dataset fact sheets](docs/datasets/index.md)). In the future it will include other common public data sources such as NREL's ATB.
